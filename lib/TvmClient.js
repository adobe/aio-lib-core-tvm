 ...
 * @type {object}
 * @property {object} cosmosClientArgs object to pass to cosmos client in cosmos sdk
 * @property {string} cosmosClientArgs.endpoint resource endpoint
 * @property {object} cosmosClientArgs.resourceTokens tokens
 * @property {string} databaseId database where container lives
 * @property {string} containerId accessible container
 * @property {string} expiration expiration date ISO/UTC
 *
 */
...
class TvmClient {
   ...
   * const client = new cosmos.CosmosClient(tvmResponse.cosmosClientArgs)
   * const database = client.database(tvmResponse.databaseId)
   * const container = database.container(tvmResponse.containerId)
   * const item = await container.items.create({ id: 'hello-item', content: 'Hello World!' })
   * ```
   *
