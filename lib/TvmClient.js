...
const { ERROR_RESPONSE, ERROR_BAD_ARGUMENT } = require('./TvmError').codes
...
class TvmClient {
  ...
  constructor (config) {
    ...
    if (res.error) {
      if (config.ow) config.ow.auth = hideAuth(config.ow.auth)

      const e = new ERROR_BAD_ARGUMENT({
        sdkDetails: { config },
  ...
  static async init (config = {}) { // no need for async now, but let's keep it consistent with rest of sdk
    ...
    logger.debug(`initializing with config: ${JSON.stringify({ ...config, ow: { namespace, auth: hideAuth(auth) } }, null, 2)}`)
  ...
  async _cacheCredentialsToFile (cacheKey, creds) {
    ...
    logger.debug(`wrote credentials to cacheFile with key ${cacheKey}`)
  ...
  async _getCredentialsFromCacheFile (cacheKey) {
    ...
    if (Date.now() > (Date.parse(creds.expiration) - 60000)) {
      logger.debug(`credentials in cacheFile with key ${cacheKey} are expired`)
      return null
    }
    logger.debug(`read valid credentials from cacheFile with key ${cacheKey}`)
    return creds
  }
