...
const retryRequest = require('../util/exponential-backoff')
...
class TvmClient {
  ...
  _cacheCredentials (cacheKey, creds) {
    if (!TvmClient[TvmClient.CredsCacheVar]) TvmClient[TvmClient.CredsCacheVar] = {}

    TvmClient[TvmClient.CredsCacheVar][cacheKey] = creds
    logger.debug(`wrote credentials to cache with key ${cacheKey}`)
  }

  _getCredentialsFromCache (cacheKey) {
    if (!TvmClient[TvmClient.CredsCacheVar]) return null

    const creds = TvmClient[TvmClient.CredsCacheVar][cacheKey]
...
TvmClient.CredsCacheVar = 'credsCache'
