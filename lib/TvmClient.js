...
class TvmClient {
  ...
  async _getPresignCredentials (endpoint, options) {
    if (!options || !options.expiryInSeconds || !options.blobName) {
      logAndThrow(new codes.ERROR_MISSING_OPTION({ messageValues: ['expiryInSeconds or blobName'], sdkDetails: options }))
    }
    let fullUrl = urlJoin(this.apiUrl, endpoint, this.ow.namespace)
    fullUrl = fullUrl + '?' + EXPIRY_IN_SECONDS + '=' + options.expiryInSeconds +
     '&' + BLOB_NAME + '=' + options.blobName

    const response = await retryRequest.exponentialBackoff(fullUrl, { headers: { Authorization: this.ow.auth } }, this.retryOptions)
    if (response.ok) {
      logger.debug(`successfully fetched presign credentials from tvm for ${fullUrl}`)
      return response.json()
    }
    const errorBody = await response.text()
    logAndThrow(new codes.ERROR_RESPONSE({
   ...
   * @throws {codes.ERROR_RESPONSE}
   */
  async getAzureBlobPresignCredentials (options) { return this._getPresignCredentials(TvmClient.AzurePresignEndpoint, options) }
}
