...
const retryRequest = require('./exponential-backoff')

const PRESIGN_REQ = 'presignReq'
const EXPIRATION_DURATION = 'expirationDuration'
 ...
 * For eg:- with a maxRetries of 5 and retryMultiplier of 100(ms), the 5 retries will have delays of 100, 200, 400, 800 and 1600 respectively.
 *
 * @typedef RetryOptions
 * @type {object}
 * @property {number} maxRetries max number of retries
 * @property {number} retryMultiplier retry multiplier in milliseconds (increases exponentially for every retry)
 */
...
class TvmClient {
   ...
   * @param  {RetryOptions} [config.retryOptions] RetryOptions. Defaults to { maxRetries: 5, retryMultiplier: 100}
  ...
  async _getCredentialsFromTVM (url, params = {}) {
    ...
    let tvmURL = urlWithNamespace
    if(params.PRESIGN_REQ) {
      tvmURL = urlWithNamespace + '&' + PRESIGN_REQ + '=true&' + EXPIRATION_DURATION + '=' + params.expirationDuration
    }

    const response = await retryRequest.exponentialBackoff(tvmURL, { headers: { Authorization: this.ow.auth } }, this.retryOptions)
    if (response.ok) {
  ...
  async _getCredentials (endpoint) {
    ...
    if (!creds) { creds = await this._getCredentialsFromCacheFile(cacheKey) }
    if (!creds) {
  ...
  async _getPresignCredentials (endpoint, options) {
    const fullUrl = urlJoin(this.apiUrl, endpoint)
    return await this._getCredentialsFromTVM(fullUrl, { PRESIGN_REQ: true, EXPIRATION_DURATION: options.expirationDuration })
  }
   ...
   * @throws {codes.ERROR_RESPONSE}
   */
  async getAzureBlobPresignCredentials (options) { return this._getPresignCredentials(TvmClient.AzureBlobEndpoint, options) }
