...
const retryRequest = require('../util/exponential-fallback')
...
class TvmClient {
  ...
  async _getCredentialsFromTVM (url) {
    ...
    const response = await retryRequest.exponentialFallback(urlWithNamespace, { headers: { Authorization: this.ow.auth } })
    if (response.ok) {
