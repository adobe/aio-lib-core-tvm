...
class TvmClient {
  ...
  async _getCredentialsFromTVM (url, params = {}) {
    ...
    const fullUrl = urlJoin(url, this.ow.namespace) + (queryParams && `?${queryParams}`)
  ...
  async getAzureBlobPresignCredentials (options = {}) {
    if (!options.expiryInSeconds || !options.blobName) {
      logAndThrow(new codes.ERROR_MISSING_OPTION({
        messageValues: ['expiryInSeconds or blobName'],
