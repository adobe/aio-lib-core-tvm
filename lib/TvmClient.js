...
class TvmClient {
  ...
  async _getCredentialsFromTVM (url, params = {}) {
    let urlWithNamespace = urlJoin(url, this.ow.namespace)
    if (params[PRESIGN_REQ]) {
      urlWithNamespace = urlWithNamespace + '?' + PRESIGN_REQ + '=true&' + EXPIRY_IN_SECONDS + '=' + params.expiryInSeconds +
  ...
  async _getPresignCredentials (endpoint, options) {
    const fullUrl = urlJoin(this.apiUrl, endpoint)
    const args = Object.assign({ presignReq: true }, options)
    return await this._getCredentialsFromTVM(fullUrl, args)
  }
